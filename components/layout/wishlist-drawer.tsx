...
export function WishlistDrawer() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="pt-[calc(1rem+env(safe-area-inset-top,0px))] px-4 pb-3 border-b border-smoke/30 flex items-center justify-between shrink-0 bg-graphite/50">
          <div className="flex items-center gap-2">
            <h2 className="font-display text-lg font-bold text-bone">Your Wishlist</h2>
          ...
          <button
            onClick={() => setWishlistOpen(false)}
            className="w-11 h-11 flex items-center justify-center rounded-full bg-graphite border border-smoke/50 text-pearl hover:text-bone hover:border-ember transition-colors"
        ...
        <div className="flex-1 overflow-y-auto p-4 pb-[calc(1.5rem+env(safe-area-inset-bottom,0px))] custom-scrollbar">
          {savedProducts.length === 0 ? (
            <div className="empty-state h-full min-h-[300px] flex flex-col items-center justify-center text-center p-6">
              ...
              <h3 className="font-display text-xl text-pearl mb-2">No saved items.</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {savedProducts.map((product) => {
                ...
                return (
                  <div
                    key={`mobile-wish-${product.id}`}
                    className="wishlist-item group relative flex flex-col gap-3 p-3.5 bg-graphite/50 border border-smoke/40 rounded-xl"
                  >
                    <Link
                      href={`/products/${product.categorySlug}/${product.slug}`}
                      onClick={() => setWishlistOpen(false)}
                      className="block relative aspect-square w-full rounded-lg overflow-hidden bg-black/30"
                    >
                      {displayImage && (
                        <Image
                          src={displayImage}
                    ...
                    <div className="flex justify-between items-start gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="font-mono text-xs font-bold text-pearl mt-0.5">
                          {formatPrice(product.basePrice)}
                        </p>
                      </div>
                      <button
                        onClick={() => toggleItem(product.id)}
                        className="w-11 h-11 rounded-full bg-ember/20 border border-ember text-ember hover:bg-charcoal hover:border-smoke hover:text-ash transition-colors shrink-0 flex items-center justify-center"
      ...
      {/* ─── DESKTOP WISHLIST DRAWER (>= lg) — 100% Pixel-Identical to Original ─── */}
      <div className="hidden lg:flex w-full h-full relative">
        <button
          onClick={() => setWishlistOpen(false)}
          className="absolute top-10 right-10 z-[160] w-12 h-12 flex items-center justify-center rounded-full bg-graphite/50 border border-smoke text-pearl hover:bg-ember hover:border-ember hover:text-bone transition-all duration-300"
        >
            ...
            <div className="grid grid-cols-2 gap-6">
              {savedProducts.map((product) => {
                ...
                return (
                  <div
                    key={product.id}
                    className="wishlist-item group relative flex flex-col gap-4 p-6 bg-graphite/40 border border-smoke/50 hover:border-ember/50 hover:bg-graphite/80 transition-all duration-300 rounded-lg"
                  >
                    <Link
                      href={`/products/${product.categorySlug}/${product.slug}`}
                      onClick={() => setWishlistOpen(false)}
