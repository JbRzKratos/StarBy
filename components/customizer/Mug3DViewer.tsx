...
function MugModel({ product }: { product: Product }) {
  ...
  const { uploadedImage, customText, customTextColor, customTextFont, mugLayout, isMagicMugRevealed } = useCustomizerStore();
  ...
  }, [uploadedImage, customText, customTextColor, customTextFont, mugLayout, isMagicMugRevealed, mTemplate]);

  if (!mTemplate || !mTemplate.dimensions3D) return null;

  const { radius, height, material, handleOffset, handleRadius, handleTube } = mTemplate.dimensions3D;
  ...
  return (
    <group position={[0, -height / 2, 0]}>
      ...
      {/* Mug Handle */}
      {handleRadius > 0.1 && (
        <mesh position={[handleOffset[0], height / 2 + handleOffset[1], handleOffset[2]]} castShadow receiveShadow>
